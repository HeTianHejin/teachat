...
-- 为teams表添加nature字段（兼容旧数据库）
ALTER TABLE teams ADD COLUMN IF NOT EXISTS nature INTEGER DEFAULT 0;
...
CREATE TABLE project_appointments (
    ...
    payer_user_id         INTEGER NOT NULL DEFAULT 0,
    payer_team_id         INTEGER NOT NULL DEFAULT 0,
    payer_family_id       INTEGER NOT NULL DEFAULT 0,
    payee_user_id         INTEGER NOT NULL DEFAULT 0,
    payee_team_id         INTEGER NOT NULL DEFAULT 0,
    payee_family_id       INTEGER NOT NULL DEFAULT 0,
    care_team_id          INTEGER NOT NULL DEFAULT 0,
    verifier_user_id      INTEGER NOT NULL DEFAULT 0,
    verifier_family_id    INTEGER NOT NULL DEFAULT 0,
    verifier_team_id      INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE handicrafts (
    ...
    project_id            INTEGER REFERENCES projects(id),
    place_id              INTEGER REFERENCES places(id),
    venue_role            VARCHAR(32) NOT NULL DEFAULT 'provider_site' CHECK (venue_role IN ('provider_site', 'payer_site', 'third_party', 'mobile')),
...
-- 字段注释
COMMENT ON COLUMN teams.class IS '团队类型：0-系统团队，1-开放式，2-封闭式，3-线下茶会家庭临时监护团队，10-开放式草团，20-封闭式草团，31-已婉拒开放式，32-已婉拒封闭式';
COMMENT ON COLUMN teams.nature IS '团队性质：0-未知/特殊，1-职业团队，2-业余团队';
COMMENT ON COLUMN teams.tags IS '分类标签，逗号分隔，如"诗词书法,文化艺术"；职业团队必须从 industry_tags 白名单中选取';
COMMENT ON COLUMN groups.class IS '集团类型：1-开放式，2-封闭式，10-开放式草集团，20-封闭式草集团，31-已婉拒开放式草集团，32-已婉拒封闭式草集团';
COMMENT ON COLUMN groups.nature IS '集团性质：0-未知/特殊，1-职业集团，2-业余集团';
...
CREATE INDEX idx_groups_nature ON groups(nature);
