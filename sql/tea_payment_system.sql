...
COMMENT ON COLUMN tea.team_accounts.balance_grams IS '团队茶叶余额，单位为克，精确到3位小数(毫克)';
COMMENT ON COLUMN tea.team_accounts.status IS '团队账户状态: normal-正常, frozen-冻结';
COMMENT ON COLUMN tea.team_accounts.frozen_reason IS '团队账户冻结原因说明';
...
-- 创建索引
CREATE INDEX idx_tea_team_transactions_team_id ON tea.team_transactions(team_id);
CREATE INDEX idx_tea_team_transactions_type ON tea.team_transactions(transaction_type);
CREATE INDEX idx_tea_team_transactions_created_at ON tea.team_transactions(created_at);
CREATE INDEX idx_tea_team_transactions_operation_id ON tea.team_transactions(operation_id);

-- 添加表注释
COMMENT ON TABLE tea.team_transactions IS '团队茶叶交易流水记录表';
COMMENT ON COLUMN tea.team_transactions.transaction_type IS '交易类型: deposit-存入, withdraw-提取, transfer_out-转出, transfer_in-转入, system_grant-系统发放, system_deduct-系统扣除';
COMMENT ON COLUMN tea.team_transactions.balance_before IS '交易前余额';
COMMENT ON COLUMN tea.team_transactions.balance_after IS '交易后余额';
COMMENT ON COLUMN tea.team_transactions.description IS '交易描述';
COMMENT ON COLUMN tea.team_transactions.related_team_id IS '交易相关团队ID（如转账的对方团队）';
COMMENT ON COLUMN tea.team_transactions.related_user_id IS '交易相关用户ID（如操作人、审批人）';
...
-- 为现有团队创建茶叶账户（如果还没有的话）
INSERT INTO tea.team_accounts (team_id, balance_grams, status)
SELECT id, 0.000, 'normal'
FROM teams t
WHERE NOT EXISTS (
