...
ALTER TABLE tea.team.transactions 
ADD COLUMN target_type VARCHAR(1) DEFAULT 'u';

-- 添加字段注释
COMMENT ON COLUMN tea.team.transactions.target_type IS '流通对象类型: u-个人, t-团队';
...
-- 创建索引
CREATE INDEX idx_tea.team.transactions_target_type ON tea.team.transactions(target_type);

-- 更新现有记录的target_type
-- 根据related_team_id和related_user_id来判断
UPDATE tea.team.transactions 
SET target_type = 't' 
