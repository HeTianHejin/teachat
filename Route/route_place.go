...
func CollectPlace(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.ScaldingTea(util.LogError(err), "Cannot convert id to int")
		Report(w, r, "你好，茶博士表示无法理解地方的类型，请稍后再试。")
		return
	}
	//检查地方是否存在
	...
	if err := t_place.Get(); err != nil {
		util.ScaldingTea(util.LogError(err), "Cannot get place by id")
	...
	if err != nil {
		util.ScaldingTea(util.LogError(err), s_u.Id, t_place_id, "Cannot check user place")
	...
	if err != nil {
		util.ScaldingTea(util.LogError(err), "Cannot get user place count")
	...
	if err := user_place.Create(); err != nil {
		util.ScaldingTea(util.LogError(err), "Cannot collect place")
		Report(w, r, "你好，茶博士表示无法收藏地方，请稍后再试。")
		return
	}
	...
	if err != nil {
		util.ScaldingTea(util.LogError(err), "Cannot get last default place")
		Report(w, r, "你好，茶博士表示无法收藏地方，请稍后再试。")
		return
	}
	if old_default_place.Id == 0 {
		...
		if err = udp.Create(); err != nil {
			util.ScaldingTea(util.LogError(err), "Cannot create user default place")
			Report(w, r, "你好，茶博士表示无法收藏地方，请稍后再试。")
			return
		}
...
func CreatePlace(w http.ResponseWriter, r *http.Request) {
	...
	if err = place.Create(); err != nil {
		util.ScaldingTea(util.LogError(err), "Cannot create place")
	...
	if err = up.Create(); err != nil {
		util.ScaldingTea(util.LogError(err), "cannot create user-place")
...
func MyPlace(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.ScaldingTea(util.LogError(err), "Cannot get places from user")
...
func PlaceDetail(w http.ResponseWriter, r *http.Request) {
	...
	if err := t_place.GetByUuid(); err != nil {
		util.ScaldingTea(util.LogError(err), "Cannot get place by uuid")
