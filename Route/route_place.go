...
func PlaceCollect(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Error("Cannot convert id to int", err)
		Report(w, r, "你好，茶博士表示无法理解地方的类型，请稍后再试。")
		return
	}
	//检查地方是否存在
	...
	if err := t_place.Get(); err != nil {
		util.Error("Cannot get place by id", err)
	...
	if err != nil {
		util.Error(s_u.Id, t_place_id, "Cannot check user place")
	...
	if err != nil {
		util.Error("Cannot get user place count", err)
	...
	if err := user_place.Create(); err != nil {
		util.Error("Cannot collect place", err)
		Report(w, r, "你好，茶博士表示无法收藏地方，请稍后再试。")
		return
	}
	...
	if err != nil {
		util.Error("Cannot get last default place", err)
		Report(w, r, "你好，茶博士表示无法收藏地方，请稍后再试。")
		return
	}
	if old_default_place.Id == 0 {
		...
		if err = udp.Create(); err != nil {
			util.Error("Cannot create user default place", err)
			Report(w, r, "你好，茶博士表示无法收藏地方，请稍后再试。")
			return
		}
...
func CreatePlace(w http.ResponseWriter, r *http.Request) {
	...
	if err = place.Create(); err != nil {
		util.Error("Cannot create place", err)
	...
	if err = up.Create(); err != nil {
		util.Error("cannot create user-place", err)
...
func MyPlace(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Error("Cannot get places from user", err)
...
func PlaceDetail(w http.ResponseWriter, r *http.Request) {
	...
	if err := t_place.GetByUuid(); err != nil {
		util.Error("Cannot get place by uuid", err)
