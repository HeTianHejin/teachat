...
func PostDetail(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Error(" Cannot get post bean given post", err)
		Report(w, r, "你好，茶博士失魂鱼，未能读取专属资料。")
		return
	}
	...
	if err != nil {
		util.Error(" Cannot get thread given post", err)
	...
	if err != nil {
		util.Error(" Cannot get thread_slice given t_post", err)
		Report(w, r, "你好，茶博士失魂鱼，未能读取专属资料。")
		return
	}
	pD.ThreadBeanSlice, err = FetchThreadBeanSlice(thread_slice)
	if err != nil {
		util.Error(" Cannot get thread_bean_slice given thread_slice", err)
		Report(w, r, "你好，茶博士失魂鱼，未能读取专属资料。")
		return
	}
	...
	if err != nil {
		util.Error(quote_thread.Id, " Cannot get project given thread")
		Report(w, r, "你好，茶博士失魂鱼，未能读取专属资料。")
		return
	}
	pD.QuoteProjectBean, err = FetchProjectBean(quote_project)
	if err != nil {
		util.Error(quote_project.Id, " Cannot get project given project")
		Report(w, r, "你好，茶博士失魂鱼，未能读取专属资料。")
		return
	}
	...
	if err != nil {
		util.Error(quote_project.Id, " Cannot get objective given project")
		Report(w, r, "你好，茶博士失魂鱼，未能读取专属资料。")
		return
	}
	pD.QuoteObjectiveBean, err = FetchObjectiveBean(quote_objective)
	if err != nil {
		util.Error(quote_objective.Id, " Cannot get objective given objective")
		Report(w, r, "你好，茶博士失魂鱼，未能读取专属资料。")
		return
	}
...
func NewPostDraft(w http.ResponseWriter, r *http.Request) {
	...
	if family_id != 0 {
		...
		if err != nil {
			...
			util.Error(" Cannot get family member by family id and user id", family_id, s_u.Id)
	...
	if err != nil {
		util.Error(" Cannot get project by project id", t_proj.Id)
	...
	if err != nil {
		util.Error(" Cannot get objective by objective id", t_obje.Id)
		Report(w, r, "你好，茶博士失魂鱼，未能读取专属茶台资料。")
		return
	}
	...
	case 1:
		...
		if err = new_draft_post.Create(); err != nil {
			util.Error(s_u.Email, " Cannot create draft post")
	...
	if err = aO.Create(); err != nil {
		util.Error("Cannot create accept_object given draft_post_id", new_draft_post.Id)
...
func UpdatePost(w http.ResponseWriter, r *http.Request) {
	...
	t_post := data.Post{Uuid: uuid}
	if err = t_post.GetByUuid(); err != nil {
		util.Error(" Cannot get post detail given uuid", uuid)
		Report(w, r, "你好，茶博士失魂鱼，未能读取专属资料。")
		return
	}

	if t_post.UserId != s_u.Id {
		util.Error(" Cannot edit other user's post", err)
