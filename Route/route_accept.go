...
func PolitePost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		util.Report(w, r, "您好，(摸摸头想了又想),电脑去热带海岛度假了。")
		return
	}
	...
	if err != nil {
		...
		util.Report(w, r, "您好，(摸摸头想了又想), 你能否再给一次提示，这次该押阿根廷还是英格兰赢球？")
		return
	}
	...
	default:
		...
		util.Report(w, r, "您好，(摸摸头想了又想),中文真是博大精深。")
		return
	}
	switch care {
	case "yes", "no":
	...
	default:
		...
		util.Report(w, r, "您好，(摸摸头想了又想),请问这是火星文吗？")
		return
	}
	...
	if !u.CheckHasReadAcceptMessage(ao_id_int) {
		util.Report(w, r, "您好，(摸摸头想了又想), 这里真的可以接受无票登陆星际飞船吗？")
		return
	}
	...
	if err != nil {
		if err == sql.ErrNoRows {
			...
			if err = newAcceptance.Create(); err != nil {
				...
				util.Report(w, r, "您好，(摸摸头想了又想), 茴香豆的茴字真的有四种写法吗？")
				return
			}
			// 首次记录完成
			util.Report(w, r, "您好，谨代表茶棚礼仪委员会感谢你！出手维护茶棚的文明秩序。")
			return
		} else {
			util.Danger(err, " Cannot check acceptance by ao_id")
			util.Report(w, r, "您好，(摸摸头想了又想)，去年今日此门中，人面桃花相映红。")
			return
		}
	}
	// update旧记录
	...
	if err = oldAcceptance.Update(); err != nil {
		...
		util.Report(w, r, "您好，(摸摸头想了又想),隔岸花分一脉香。")
		return
	}
	// 读取友邻盲评邀请函
	...
	if err = acMess.GetAccMesByUIdAndAOId(u.Id, oldAcceptance.AcceptObjectId); err != nil {
		...
		util.Report(w, r, "您好，茶博士莫名其妙，竟然说没有票也可以潜水有时候是合情合理的。")
		return
	}
	...
	if err = ao.Get(); err != nil {
		...
		util.Report(w, r, "您好，(摸摸头想了又想),得道多助，失道寡助。")
		return
	}
	// 检查新茶评审结果
	if oldAcceptance.XAccept && oldAcceptance.YAccept {
		...
		case 1:
			...
			if err != nil {
				...
				util.Report(w, r, "您好，茶博士失魂鱼，竟然说没有找到新茶评审的资料未必是怪事。")
				return
			}
			if ob.Class == 10 {
				ob.Class = 1
			...
			if err = ob.UpdateClass(); err != nil {
				...
				util.Report(w, r, "您好，(摸摸头想了又想), 为什么踢足球的人都说临门一脚最麻烦呢？")
				return
			}
			// 通知茶语作者友邻盲评已通过
			//构造消息
		...
		case 2:
			...
			if err != nil {
				...
				util.Report(w, r, "您好，茶博士失魂鱼，竟然说有时找资料也是一种修养的过程。")
				return
			}
			if pr.Class == 10 {
				pr.Class = 1
			...
			if err = pr.UpdateClass(); err != nil {
				...
				util.Report(w, r, "您好，一畦春韭绿，十里稻花香。")
				return
			}
			// 通知茶语作者友邻盲评已通过
			//构造消息
		...
		case 3:
			...
			if err = dThread.Get(); err != nil {
				...
				util.Report(w, r, "您好，茶博士失魂鱼，竟然说有时候找资料需要的不是技术而是耐心。")
				return
			}
			// 更新������，友����评已通过！
			if err = dThread.UpdateClass(1); err != nil {
				util.Warning(err, "Cannot update thread class")
				util.Report(w, r, "您好，睿藻仙才盈彩笔，自惭何敢再为辞。")
				return
			}
			// ���为正式品��稿
			thread := data.Thread{
			...
			if err = thread.Save(); err != nil {
				...
				util.Report(w, r, "您好，吟成荳蔻才犹艳，睡足酴醾梦也香。")
				return
			}
			// 通知茶语作者友邻盲评已通过
			//构造消息
		...
		case 5:
			...
			if err != nil {
				...
				util.Report(w, r, "您好，茶博士失魂鱼，竟然说有时候临急抱佛脚比刻苦奋斗更有用？")
				return
			}
			if team.Class == 10 {
				team.Class = 1
			...
			if err = team.UpdateClass(); err != nil {
				...
				util.Report(w, r, "您好，（摸摸头）考一考你，情中情因情感妹妹　错里错以错劝哥哥是什么意思？")
				return
			}
			...
			if err = teamMember.Create(); err != nil {
				...
				util.Report(w, r, "您好，花因喜洁难寻偶，人为悲秋易断魂。")
				return
			}
		...
		default:
			...
			util.Report(w, r, "您好，茶博士失魂鱼，竟然说有时候什么都不做,就能赢50%的竞争对手？")
			return
		}
		// 感谢友邻维护了茶棚的文明秩序
		util.Report(w, r, "您好，好茶香护有缘人，感谢你出手维护了茶棚的文明秩序！")
		return
	} else {
		//友邻盲评拒绝接纳这个茶语！Oh my...
		...
		case 4:
			...
			if err != nil {
				util.Danger(err, "Cannot get draft-post")
				util.Report(w, r, "您好，茶博士失魂鱼，竟然说有时候 弄丢草稿的人不一定是诗人？")
				return
			}
		...
		case 5:
			...
			if err = team.UpdateClass(); err != nil {
				util.Danger(err, "Cannot update team class")
				util.Report(w, r, "您好，（摸摸头）考一考你，情中情因情感妹妹　错里错以错劝哥哥是什么茶品种？")
				return
			}
		}
		// 感谢友邻维护了茶棚的文明秩序
...
func PoliteGet(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		util.Report(w, r, "您好，换成int失败，评审的资料？")
		return
	}
	// 友邻盲评对象
	ao := data.AcceptObject{
	...
	if err = ao.Get(); err != nil {
		util.Danger(err, "Cannot get object")
		util.Report(w, r, "您好，茶博士都糊涂了，竟然唱问世间情为何物，直教人找不到对象？")
		return
	}
	...
	if err = acceptMessage.GetAccMesByUIdAndAOId(u.Id, ao.Id); err != nil {
		util.Warning(err, "Cannot get accept-message invitation")
		util.Report(w, r, "您好，茶博士莫名其妙，竟然说没有机票也可以登机有时候是合情合理的。")
		return
	}
	...
	if !u.CheckHasAcceptMessage(ao.Id) {
		...
		util.Report(w, r, "您好，莫名其妙的茶博士竟然强词夺理说，外星人不能评估新茶～")
		return
	}
	// 根据对象类型处理
	switch ao.ObjectType {
	case 1:
		...
		if err != nil {
			util.Danger(err, "Cannot get objective")
			util.Report(w, r, "您好，有时候找不到新茶评审的资料未必是外星人闹事。")
			return
		}
	...
	case 2:
		...
		if err != nil {
			util.Danger(err, "Cannot get project")
			util.Report(w, r, "您好，茶博士失魂鱼，竟然说有时找资料也是一种修心养性的过程。")
			return
		}
	...
	default:
		util.Danger(err, "Cannot get object")
		util.Report(w, r, "您好，茶博士失魂鱼，竟然说有时候什么都不做就能赢50%的竞争对手？")
		return
	}
	...
	util.GenerateHTML(w, &ao, "layout", "navbar.private", "watch_your_language")
}
