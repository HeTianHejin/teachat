...
func CreateGroup(w http.ResponseWriter, r *http.Request) {
	...
	le := CnStrLen(name)
	if le < 4 || le > 24 {
		Report(w, r, "您好，茶博士摸摸头，竟然说集团名字字数太多或者太少，未能创建新集团。")
		return
	}
	_, err = data.GetGroupByName(name)
	if err == nil {
		//重名
		Report(w, r, "您好，茶博士摸摸头，竟然说集团名称重复，未能创建新集团。")
		return
	}
	...
	lenA := CnStrLen(abbr)
	if lenA < 4 || lenA > 6 {
		Report(w, r, "您好，茶博士摸摸头，竟然说队名简称字数太多或者太少，未能创建新集团。")
		return
	}
	_, err = data.GetGroupByAbbreviation(abbr)
	if err == nil {
		//重名
		Report(w, r, "您好，茶博士摸摸头，竟然说集团简称重复，未能创建新集团。")
		return
	}
	...
	case 10, 20:
		break
	default:
		Report(w, r, "您好，茶博士摸摸头，竟然说集团类别太多或者太少，未能创建新集团。")
		return
	}
	team, err := su.GetLastDefaultTeam()
	if err != nil {
		util.Info(err, "Cannot get last default team")
		Report(w, r, "您好，茶博士彬彬有礼的说，找不到您的默认团队信息，请稍后再试。")
		return
	}
	...
	if err != nil {
		...
		Report(w, r, "您好，茶博士因为的笔没有墨水了，未能创建新集团，请稍后再试。")
		return
	}
	...
	t := fmt.Sprintf("您好，新集团 %s 已准备妥当，稍等有缘茶友评审通过之后，即行昭告天下。", g.Abbreviation)
	// 提示用户草稿保存成功
	Report(w, r, t)
}
...
func GroupDetail(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		Report(w, r, "您好，茶博士摸摸头，居然说集团可能存在黑洞，未能查看集团详情。")
		return
	}
	// 读取第一管理团队
	...
	if err != nil {
		Report(w, r, "您好，����������头，��然说集��可能存在黑��，未能查看集��详情。")
		return
	}
