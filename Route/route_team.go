...
func SetDefaultTeam(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Debug("Cannot get team by given uuid %v", err)
	...
	if err != nil {
		util.Debug("Cannot get last default team %v", err)
		report(w, s_u, "你好，茶博士失魂鱼，未能获取茶团，请稍后再试。")
		return
	}
	...
	if err != nil {
		util.Debug("Cannot check user is member of team %v", err)
		report(w, s_u, "你好，茶博士失魂鱼，未能获取茶团，请稍后再试。")
		return
	}
	if !ok {
	...
	if err = new_default_team.Create(); err != nil {
		util.Debug("Cannot set default team %v", err)
		report(w, s_u, "你好，茶博士失魂鱼，未能设置默认茶团，请稍后再试。")
		return
	}
...
func CreateTeamPost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Debug("connot count teams given founder_id %v", err)
	...
	l := cnStrLen(new_name)
	if l < 4 || l > 24 {
		report(w, s_u, "你好，茶博士摸摸头，竟然说茶团名字字数太多或者太少，未能创建新茶团。")
		return
	}
	...
	lenA := cnStrLen(abbr)
	if lenA < 4 || lenA > 6 {
		report(w, s_u, "你好，茶博士摸摸头，竟然说队名简称字数太多或者太少，未能创建新茶团。")
		return
	}
	...
	if lenM < int(util.Config.ThreadMinWord) || lenM > int(util.Config.ThreadMaxWord) {
		report(w, s_u, "你好，茶博士摸摸头，竟然说愿景字数太多或者太少，未能创建新茶团。")
		return
	}
	class, err := strconv.Atoi(r.PostFormValue("class"))
	if err != nil {
		util.Debug(" Cannot convert class to int %v", err)
		report(w, s_u, "你好，茶博士失魂鱼，未能创建新茶团，请稍后再试。")
		return
	}
	...
	if err != nil {
		util.Debug(" Cannot convert nature to int %v", err)
		report(w, s_u, "你好，茶博士失魂鱼，未能创建新茶团，请稍后再试。")
		return
	}
	switch nature {
	...
	if strings.Contains(new_name, "自由人") || strings.Contains(new_name, "Freelancer") {
		//不允许，以免误导其他茶友
		report(w, s_u, "你好，茶团名称不能包含“自由人”这样的保留关键字，请换一个更响亮的团名，再试一次。")
		return
	}
	if err = old_team.GetByAbbreviation(); err == nil {
		// 重复的简称,不允许，以免误导其他茶友
		report(w, s_u, "你好，茶博士摸摸头，竟然说这个茶团简称已经被占用，请换一个更响亮的团名，再试一次。")
		return
	}
	//检测abbr中是否包含“自由人”或者“Freelancer”.
	if strings.Contains(abbr, "自由人") || strings.Contains(abbr, "Freelancer") {
		//不允许使用的保留关键字
		report(w, s_u, "你好，茶团简称不能包含“自由人”或者“Freelancer”这样的保留关键字，请换一个更响亮的团名简称，再试一次。")
		return
	}
	...
	if err := createTeamWithFounderMember(&new_team, s_u.Id); err != nil {
		util.Debug("cannot create team with founder member %v", err)
		report(w, s_u, "你好，茶博士失魂鱼，暂未能创建你的天命使团，请稍后再试。")
		return
	}
...
func HoldTeams(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Debug(" Cannot get hold teams %v", err)
		return
	}
	...
	if len(filtered_teams) == 0 {
		...
		if err != nil {
			util.Debug(" Cannot get team bean slice %v", err)
...
func JoinedTeams(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Debug(" Cannot get joined teams %v", err)
	...
	if len(teamBeanSlice) > 1 {
		...
		if err != nil {
			util.Debug(" Cannot move default team to front %v", err)
			report(w, s_u, "你好，茶博士未能帮忙查看茶团，请稍后再试。")
			return
		}
	}
...
func EmployedTeams(w http.ResponseWriter, r *http.Request) {
	...
	team_slice, err := s_u.CoreExecTeams()
	if err != nil {
		util.Debug(" Cannot get employed teams %v", err)
		report(w, s_u, "你好，茶博士必须先找到自己的高度近视眼镜，再帮您查询资料。请稍后再试。")
		return
	}
...
func TeamDetail(w http.ResponseWriter, r *http.Request) {
	...
	if team.IsPrivate {
		...
		if err != nil {
			util.Debug(" Cannot check if user is member of team %v", err)
			report(w, s_u, "你好，茶博士说久仰大名，请问大名是谁？")
			return
		}
		if !isMember {
	...
	if err != nil {
		util.Debug(" Cannot get team bean %v", err)
	...
	if err != nil {
		util.Debug(" Cannot get team core member %v", err)
		report(w, s_u, "你好，茶博士为你效劳中，请稍后再试。")
		return
	}
	teamNormalMembers, err := team.NormalMembers()
	if err != nil {
		util.Debug(" Cannot get team normal member given team uuid %v", err)
		report(w, s_u, "你好，茶博士为你效劳中，请稍后再试。")
		return
	}
	...
	for _, member := range teamCoreMembers {
		...
		if err != nil {
			util.Debug(" Cannot get user's default family %v", err)
			report(w, s_u, "你好，满头大汗的茶博士，开口唱蝶恋花，请稍后再试。")
			return
		}

		tc.MemberDefaultTeam, err = cm_user.GetLastDefaultTeam()
		if err != nil {
			util.Debug(" Cannot get user's default team %v", err)
			report(w, s_u, "你好，闪电茶博士为你效劳中，请稍后再试。")
			return
		}
	...
	for _, member := range teamNormalMembers {
		...
		if err != nil {
			util.Debug(" Cannot get user's default family %v", err)
			report(w, s_u, "你好，满头大汗的茶博士说小生这边有礼了，请稍后再试。")
			return
		}
		tn.MemberDefaultTeam, err = tn_user.GetLastDefaultTeam()
		if err != nil {
			util.Debug(" Cannot get user's default team %v", err)
			report(w, s_u, "你好，茶博士为你效劳，请稍后再试。")
			return
		}
	...
	if tD.IsCoreMember {
		...
		if err != nil {
			util.Debug("Cannot get member application count %v", err)
			report(w, s_u, "你好，茶博士未能帮忙查看茶团，请稍后再试。")
			return
		}
		if count > 0 {
		...
		if err != nil {
			util.Debug("Cannot get resignations %v", err)
		} else {
...
func ManageTeamIndexGet(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Debug(" Cannot get this team %v", err)
	...
	if s_u.Id == team.FounderId {
		...
		if err != nil {
			util.Debug(" Cannot get ceo of this team %v", err)
			report(w, s_u, "你好，茶博士未能找到此茶团资料，请确认后再试。")
			return
		}
	...
	if is_manager {
		...
		report(w, s_u, "你好，茶博士认为，您今天无权管理此茶团。")
		return
	}
	...
	if err != nil {
		util.Debug(" Cannot get FetchTeamMemberBeanSlice() %v", err)
		report(w, s_u, "你好，茶博士为你疯狂效劳中，请稍后再试。")
		return
	}
	...
	if len(tCMBSlice) == 0 {
		report(w, s_u, "你好，茶博士未能找到此茶团核心成员资料，请确认后再试。")
		return
	}
...
func CoreManage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Debug(" Cannot strconv this team_id %v", err)
		report(w, s_u, "你好，茶博士未能找到此茶团资料，请确认后再试。")
		return
	}
	//这个茶团是否存在？
	...
	if err != nil {
		util.Debug(" Cannot convert member_id to int %v", err)
	...
	if err != nil {
		util.Debug(" Cannot get this team member %v", err)
		report(w, s_u, "你好，茶博士未能找到此茶团成员资料，请确认后再试。")
		return
	}
	...
	case "appoint", "discharge":
		...
		if err != nil {
			report(w, s_u, "你好，保存角色管理操作失败，请稍后再试。")
			return
		}
	default:
		report(w, s_u, "你好，请选择正确的管理动作。")
		return
	}
...
func teamLogoUploadPost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Debug("fail to get team by uuid %v", err)
	...
	if ok {
		...
		if errAvatar == nil {
			...
			util.Debug("fail to save upload team logo image %v", errAvatar)
...
func TeamApplications(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Debug("Cannot get applications %v", err)
	...
	if err != nil {
		util.Debug("Cannot fetch application beans %v", err)
		report(w, s_u, "你好，茶博士正在努力的查找申请书，请稍后再试。")
		return
	}
...
func TeamInvitations(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		report(w, s_u, "你好，茶博士正在努力的查找茶团发送的邀请函，请稍后再试。")
		return
	}
...
func canManageTeam(team *dao.Team, s_u dao.User, w http.ResponseWriter) bool {
	...
	if err != nil {
		util.Debug("Cannot get core members %v", err)
		report(w, s_u, "你好，茶博士失魂鱼，居然说这个茶团是由外星人组织的，请确认后再试。")
		return false
	}
	...
	if !isCoreMember {
		report(w, s_u, "你好，蛮不讲理的茶博士竟然说，只有茶团核心成员才能查看此内容。")
		return false
	}
...
func EditTeamGet(w http.ResponseWriter, r *http.Request) {
	...
	if !canEdit {
		report(w, s_u, "你好，只有茶团创建人或CEO才能编辑茶团资料。")
		return
	}
...
func EditTeamPost(w http.ResponseWriter, r *http.Request) {
	...
	if nameLen < 4 || nameLen > 24 {
		report(w, s_u, "你好，茶团名称应在4-24个中文字符之间。")
		return
	}
	...
	if abbrLen < 4 || abbrLen > 6 {
		report(w, s_u, "你好，队名简称应在4-6个中文字符之间。")
		return
	}
	...
	if err := team.Update(); err != nil {
		util.Debug("Cannot update team %v", err)
		report(w, s_u, "你好，茶博士失魂鱼，未能更新茶团信息。")
		return
	}
...
func TeamMemberAddGet(w http.ResponseWriter, r *http.Request) {
	...
	if !canManage {
		report(w, s_u, "你好，只有茶团创建人或CEO才能邀请新成员。")
		return
	}
...
func createTeamWithFounderMember(team *dao.Team, founderUserId int) error {
	...
	if err := dao.EnsureTeaTeamAccountExists(team.Id); err != nil {
		util.Debug(" Cannot create team tea account %v", err)
		return err
	}
