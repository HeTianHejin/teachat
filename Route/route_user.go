...
func Biography(w http.ResponseWriter, r *http.Request) {
	...
	s_u, err := s.User()
	if err != nil {
		util.Warning(util.LogError(err), " 根据会话未能读取用户信息")
	...
	if err != nil {
		util.Warning(util.LogError(err), " 未能读取用户信息！")
		Report(w, r, "你好，茶博士失魂鱼，未能读取用户信息.")
		return
	}
...
func EditIntroAndName(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		err = r.ParseForm()
		if err != nil {
			util.Warning(util.LogError(err), "解析表单错误！")
		}
		s_u, _ := s.User()
		...
		if err != nil {
			util.Warning(util.LogError(err), " 更新用户信息错误！")
...
func SaveAvatar(w http.ResponseWriter, r *http.Request) {
	...
	s_u, err := s.User()
	if err != nil {
		util.Warning(util.LogError(err), " 获取用户信息错误！")
		Report(w, r, "你好，茶博士失魂鱼，未能读取用户信息！")
		return
	}
	// 处理上传到图片
