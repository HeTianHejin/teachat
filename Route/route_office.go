...
func Invite(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.PanicTea(util.LogError(err), " Cannot fetch user related data")
	...
	if err != nil {
		util.PanicTea(util.LogError(err), " Cannot get user by uuid")
		Report(w, r, "你好，柳丝榆荚自芳菲，不管桃飘与李飞。请稍后再试。")
		return
	}
...
func TwoAcceptMessagesSendExceptUserId(u_id int, mess data.AcceptMessage) error {
	...
	// }
	if len(user_ids) != 2 {
		util.PanicTea(util.LogError(err), " Cannot get 2 random-user-ids")
		return err
	}
	// 发送“是否接纳”消息
	if err = mess.Send(user_ids); err != nil {
		util.PanicTea(util.LogError(err), " Cannot send accept message")
		return err
	}
	// 记录用户有1新消息
	for _, u_id := range user_ids {
		if err = data.AddUserMessageCount(u_id); err != nil {
			util.PanicTea(util.LogError(err), " Cannot add random user new-message-count")
			return err
		}
	}
	return nil
