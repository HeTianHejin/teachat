...
func MemberResignPost(w http.ResponseWriter, r *http.Request) {
	...
	if lenTit < 2 || lenTit > 64 {
		report(w, s_u, "你好，茶博士认为标题字数太长或者太短，请确认后再试。")
		return
	}
	...
	if ok := isEmail(m_email); !ok {
		report(w, s_u, "你好，涨红了脸的茶博士，竟然强词夺理说，电子邮箱格式太复杂看不懂，请确认后再提交。")
		return
	}
	//读取声明退出的成员资料
	...
	//检查提交人是否和会话用户一致
	if s_u.Id != t_user.Id {
		report(w, s_u, "你好，目前不能代替他人提交退出茶团声明，请确认后再试。")
		return
	}
	...
	// 读取目标茶团资料
	t_team, err := dao.GetTeam(team_id)
	if err != nil {
		util.Error("Cannot get team by id %d: %v", team_id, err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
	// 检查提交人是否为茶团成员，
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			...
			util.Error("Cannot get team member by team_id %d and user_id %d: %v", t_team.Id, t_user.Id, err)
	...
	tmqD := dao.TeamMemberResignation{
		...
		Title:             titl,
		Content:           cont,
		Status:            dao.ResignationStatusUnread,
	}
...
func MemberResignGet(w http.ResponseWriter, r *http.Request) {
	...
	//读取目标茶团资料
	t_team, err := dao.GetTeamByUUID(team_uuid)
	if err != nil {
		util.Error("Cannot get team by uuid %s: %v", team_uuid, err)
		report(w, s_u, "你好，满头大汗的茶博士表示找不到这个茶团，稍后再试。")
		return
	}
	...
	if err != nil {
		util.Error("Cannot check if user %d is active member of team %d: %v", s_u.Id, t_team.Id, err)
...
func MemberRoleChanged(w http.ResponseWriter, r *http.Request) {
	...
	role_notices, err := dao.GetMemberRoleNoticesByTeamId(t_team.Id)
	if err != nil {
		util.Error("Cannot get team member role change notice by team id %d: %v", t_team.Id, err)
		report(w, s_u, "你好，茶博士正在忙碌中，请稍后再试。")
		return
	}
	tmrnBeanSlice, err := fetchTeamMemberRoleNoticeBeanSlice(role_notices)
	if err != nil {
		util.Error("Cannot fetch team member role notice bean slice: %v", err)
		report(w, s_u, "你好，茶博士正在忙碌中，请稍后再试。")
		return
	}

	var tmrcnpd dao.TeamMemberRoleChangedNoticesPageData
...
func MemberRoleChange(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Error("Cannot convert team_id %s to int: %v", team_id_str, err)
		report(w, s_u, "你好，茶博士的眼镜被闪电破坏了，请稍后再试。")
		return
	}
	...
	if ok := isEmail(member_email); !ok {
		report(w, s_u, "你好，茶博士的眼镜被闪电破坏了，看不清提及的邮箱，请稍后再试。")
		return
	}

	//读取目标茶团资料
	t_team, err := dao.GetTeam(team_id_int)
	if err != nil {
		util.Error("Cannot get team by id %d: %v", team_id_int, err)
		report(w, s_u, "你好，满头大汗的茶博士表示找不到这个茶团，稍后再试。")
		return
	}
	//读取拟调整角色目标茶友资料
	t_member, err := dao.GetUserByEmail(member_email, r.Context())
	if err != nil {
		util.Error("Cannot get user given email %s: %v", member_email, err)
		report(w, s_u, "你好，满头大汗的茶博士表示找不到这个茶友，稍后再试。")
		return
	}

	//检查目标茶友是否茶团成员
	_, err = dao.GetMemberByTeamIdUserId(t_team.Id, t_member.Id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			report(w, s_u, "你好，满头大汗的茶博士表示这个不是茶团的成员，稍后再试。")
			return
		} else {
			util.Error("when GetMemberByTeamIdAndUserId() checking team_member: %v", err)
			report(w, s_u, "你好，茶博士的眼镜被闪电破坏了，请稍后再试。")
			return
		}
	}
	...
	if err != nil {
		util.Error("Cannot get team ceo given team id %d: %v", t_team.Id, err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
	...
	if err != nil {
		util.Error("Cannot get team founder given team id %d: %v", t_team.Id, err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
	}
	// 准备资料
	var tmrcnP dao.TeamMemberRoleChangeNoticePage
	is_manager := false
	...
	//读取CEO茶友资料
	t_ceo, err := dao.GetUser(member_ceo.UserId)
	if err != nil {
		util.Error("Cannot get user by id %d: %v", member_ceo.UserId, err)
		report(w, s_u, "你好，茶博士正在忙碌中，请稍后再试。")
		return
	}

	m_c_role, err := dao.GetTeamMemberRoleByTeamIdAndUserId(t_team.Id, t_member.Id)
	if err != nil {
		util.Error("Cannot get team member role given team id %d: %v", t_team.Id, err)
		report(w, s_u, "你好，茶博士正在忙碌中，请稍后再试。")
		return
	}
...
func MemberRoleReply(w http.ResponseWriter, r *http.Request) {
	...
	if new_role == dao.RoleCEO || new_role == dao.RoleCTO || new_role == dao.RoleCMO || new_role == dao.RoleCFO {
		...
		if err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				...
				util.Error("Cannot get team member by role %d: %v", new_role, err)
				report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
				return
			}
		} else {
	...
	t_user, err := dao.GetUserByEmail(m_email, r.Context())
	if err != nil {
		util.Error("Cannot get user by email %s: %v", m_email, err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
	...
	if err != nil {
		util.Error("Cannot check if user %d is active member of team %d: %v", t_user.Id, t_team.Id, err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
	if !isActiveMember {
	...
	if cnStrLen(title) < 6 || cnStrLen(title) > 24 {
		report(w, s_u, "你好，茶博士摸摸头嘀咕说，你提交的标题太长或太短，请确认后再提交。")
		return
	}
	//提交的角色调整内容
	...
	if cnStrLen(content) < 2 || cnStrLen(content) > int(util.Config.ThreadMaxWord) {
		report(w, s_u, "你好，茶博士摸摸头嘀咕说，你提交的内容太长或太短，请确认后再提交。")
		return
	}
	...
	if m_ceo.UserId == s_u.Id && new_role != dao.RoleCEO {
		...
		if err = team_member_role_notice.Create(); err != nil {
			util.Error("Cannot create team_member_role_notice: %v", err)
			report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
			return
		}
		//更新成员角色
		...
		if err = member.UpdateRoleStatus(); err != nil {
			util.Error("Cannot update member: %v", err)
			report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
			return
		}
		...
		if new_role == dao.RoleCEO {
			...
			if err != nil {
				util.Error("Cannot get founder member given team %d and user %d: %v", t_team.Id, t_founder.Id, err)
				report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
				return
			}
			founder_member.Status = dao.TeamMemberStatusActive
			if err = founder_member.UpdateRoleStatus(); err != nil {
				util.Error("Cannot update founder member: %v", err)
				report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
				return
			}
		}
...
func MemberApplicationDetail(w http.ResponseWriter, r *http.Request) {
	...
	application := dao.MemberApplication{Uuid: application_uuid}
	if err = application.GetByUuid(); err != nil {
		util.Error("cannot get application given uuid %s: %v", application_uuid, err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}

	applicant, err := dao.GetUser(application.UserId)
	if err != nil {
		util.Error("Cannot get user given id %d: %v", application.UserId, err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}

	team, err := dao.GetTeam(application.TeamId)
	if err != nil {
		util.Error("Cannot get team given id %d: %v", application.TeamId, err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
	//权限检查，用户可以查看自己的申请书详情，或者是团队管理员可以
	if applicant.Id != s_u.Id && !canManageTeam(&team, s_u, w) {
		return
	}

	applicant_default_team, err := applicant.GetLastDefaultTeam()
	if err != nil {
		util.Error("Cannot get default team given user %d: %v", applicant.Id, err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}

	var aR dao.ApplicationReview
	aR.SessUser = s_u
...
func MemberApplicationReply(w http.ResponseWriter, r *http.Request) {
	...
	s_u, err := s.User()
	if err != nil {
		util.Error("Cannot get user from session %s: %v", s.Email, err)
		http.Redirect(w, r, "/v1/login", http.StatusFound)
		return
	}
	//解析表单内容，获取茶友提交的内容
	err = r.ParseForm()
	if err != nil {
		util.Error("Cannot parse form: %v", err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
	...
	if err != nil {
		util.Error("Cannot convert application_id %s to int: %v", application_id_str, err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
	application := dao.MemberApplication{
		Id: application_id,
	}
	//读取加盟茶团申请书
	if err = application.Get(); err != nil {
		util.Error("Cannot get application given id %d: %v", application_id, err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
	//检查申请书的状态是否正常，已查看
	switch application.Status {
	case dao.MemberApplicationStatusPending:
		//未查看
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	case dao.MemberApplicationStatusViewed:
		//已查看，但未处理
		break
	case dao.MemberApplicationStatusApproved, dao.MemberApplicationStatusRejected:
		// 已经处理完成
		report(w, s_u, "你好，这份申请书已经被处理，请确认后再试。")
		return
	case dao.MemberApplicationStatusExpired:
		//已经过期或者失效
		report(w, s_u, "你好，这份申请书已经过期或者失效，请确认后再试。")
		return
	default:
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
	}
	...
	if err != nil {
		util.Error("Cannot get core members of team %d: %v", team.Id, err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
	// 如果不是茶团的核心成员，返回错误
	if !is_core_member {
		report(w, s_u, "你好，茶博士摸摸头嘀咕说，你不是茶团的核心成员，无权处理申请书噢。")
		return
	}
	...
	_, err = dao.GetMemberByTeamIdUserId(team.Id, applicant.Id)
	if err != nil {
		switch err {
		case sql.ErrNoRows:
			break
		default:
			util.Error("when checking team_member: %v", err)
			report(w, s_u, "你好，茶博士的眼镜被闪电破坏了，请稍后再试。")
			return
		}
	} else {
		report(w, s_u, "你好，茶博士摸摸头嘀咕说，茶友你已经在茶团中了噢？请确认后再试。")
		return
	}
	...
	if reply == "" {
		report(w, s_u, "你好，茶博士摸摸头嘀咕说，你没有填写回复内容或者内容太复杂了。")
		return
	}
	...
	case 1:
		...
		if err = team_member.Create(); err != nil {
			util.Error("Cannot create team_member %v", err)
			report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
			return
		}
		...
		if err != nil {
			util.Error("Cannot get survival teams count for user %d: %v", applicant.Id, err)
			report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
			return
		}
		if count == 1 {
			...
			if err = user_default_team.Create(); err != nil {
				util.Error("Cannot create user_default_team for user %d: %v", applicant.Id, err)
				report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
				return
			}
		}
		...
		//更新申请书的状态为已批准
		application.Status = dao.MemberApplicationStatusApproved
		if err = application.Update(); err != nil {
			util.Error("Cannot update application %v", err)
			report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
			return
		}
		//创建批准加盟申请书答复
		application_reply := dao.MemberApplicationReply{
			MemberApplicationId: application.Id,
			TeamId:              team.Id,
			UserId:              s_u.Id,
			ReplyContent:        reply,
			Status:              dao.MemberApplicationStatusApproved,
		}
		if err = application_reply.Create(); err != nil {
			util.Error("Cannot create application_reply %v", err)
			report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
			return
		}

		//报告批准加盟成功消息
		report(w, s_u, "你好，已经批准新成员 "+applicant.Name+" 加盟 "+team.Name+" 茶团。")
		return
	case 0:
		//婉拒加盟
		application.Status = dao.MemberApplicationStatusRejected
		if err = application.Update(); err != nil {
			util.Error("Cannot update application %v", err)
			report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
			return
		...
		application_reply := dao.MemberApplicationReply{
			...
			UserId:              s_u.Id,
			ReplyContent:        reply,
			Status:              dao.MemberApplicationStatusRejected,
		}
		if err = application_reply.Create(); err != nil {
			util.Error("Cannot create application_reply %v", err)
			report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		...
		report(w, s_u, "你好，茶团成员 "+applicant.Email+" 已经婉拒加盟茶团 "+team.Name+"。")
		return
	default:
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
...
func MemberApplicationReview(w http.ResponseWriter, r *http.Request) {
	...
	if application.Status > dao.MemberApplicationStatusViewed {
	...
	if err != nil {
		...
		report(w, s_u, "你好，这个申请人已经在茶团中了噢？请确认后再试。")
		return
	}
	// 读取申请人默认所在的茶团
	applicant_default_team, err := applicant.GetLastDefaultTeam()
	...
	//如果不是茶团的核心成员，返回错误
	if !is_core_member {
		report(w, s_u, "你好，茶博士摸摸头嘀咕说，你不是茶团的核心成员，无权审查申请书噢。")
		return
	}
	...
	if err = application.Update(); err != nil {
		util.Error("Cannot update application status for application %d: %v", application.Id, err)
	}
...
func NewMemberApplication(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Error("Cannot get team given uuid %s: %v", team_uuid, err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
	...
	if len(content) > 666 || len(content) < 2 {
		report(w, s_u, "你好，茶博士摸摸头嘀咕说，茶友你的申请书内容太长了噢？墨水瓶也怕抄单词呀！")
		return
	}

	// s_u.Id != user.Id，检查是否茶友本人提交申请，不允许代他人申请
	if s_u.Id != app_user.Id {
		report(w, s_u, "你好，身前有余忘缩手，眼前无路想回头，目前仅接受本人申请加入茶团噢。")
		return
	}

	//检查这个茶团是否已经存在该茶友
	_, err = dao.GetMemberByTeamIdUserId(team.Id, app_user.Id)
	if err != nil {
		switch err {
		case sql.ErrNoRows:
			break
	...
	if err = ma.Create(); err != nil {
		util.Error("Cannot create member-application: %v", err)
		report(w, s_u, "你好，茶博士正在飞速处理所有的技术问题，请耐心等待。")
		return
	}
...
func NewMemberApplicationForm(w http.ResponseWriter, r *http.Request) {
	...
	_, err = dao.CheckMemberApplicationByTeamIdAndUserId(team.Id, s_u.Id, dao.MemberApplicationStatusViewed)
	if err != nil {
		switch err {
		case sql.ErrNoRows:
			break
		default:
			util.Error(" when checking member_application: %v", err)
			report(w, s_u, "你好，茶博士的眼镜被闪电破坏了，请稍后再试。")
			return
		}
	} else {
		report(w, s_u, "你好，茶博士摸摸头嘀咕说，茶友你已经提交过申请书了噢？请确认后再试。")
		return
	}
	...
	if err != nil {
		util.Error("Cannot check team member: %v", err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
...
func MemberInvitationReply(w http.ResponseWriter, r *http.Request) {
	...
	s_u, err := s.User()
	if err != nil {
		util.Error("Cannot get user from session: %v", err)
	...
	//检查一下邀请函是否已经被回复
	if invitation.Status > dao.InvitationStatusViewed {
	...
	if invi_user.Id != s_u.Id {
		report(w, s_u, "你好，茶博士摸摸头嘀咕说，这个邀请函不是你的，你无权回答。")
		return
	}
	...
	if reply_word == "" || cnStrLen(reply_word) > int(util.Config.ThreadMaxWord) {
		...
		report(w, s_u, "你好，瞪大眼睛涨红了脸的茶博士，竟然强词夺理说，答复的话太长了或者太短，只有外星人才接受呀，请确认再试。")
		return
	}
	...
	if reply_class_int == 1 {
		...
		if team_member.Role == dao.RoleCEO {
			...
			if err = team_member.Create(); err != nil {
				...
				report(w, s_u, "你好，晕头晕脑的茶博士竟然忘记登记新成员了，请稍后再试。")
				return
			}
		}
		...
		invitation.Status = dao.InvitationStatusRejected
		if err = invitation.UpdateStatus(); err != nil {
			util.Debug(s_u.Email, " Cannot update invitation status", err)
			report(w, s_u, "你好，晕头晕脑的茶博士竟然把邀请答复处理搞混了，请稍后再试。")
			return
		}
		repl := dao.InvitationReply{
			InvitationId: invitation_id,
		...
		if err = repl.Create(); err != nil {
			...
			report(w, s_u, "你好，晕头晕脑的茶博士竟然把邀请答复搞丢了，请稍后再试。")
			return
		}
		//报告用户已经保存拒绝该邀请到答复记录
		...
		// 无效的reply 数值
		report(w, s_u, "你好，何幸邀恩宠，宫车过往频。稍后再试。")
		return
	}
}
...
func InviteMemberPost(w http.ResponseWriter, r *http.Request) {
	...
	ok := founder.Id == s_u.Id ||
		s_u.Id == ceo_user.Id
	if !ok {
		report(w, s_u, "你好，只有团队CEO或者创建人可以邀请请新成员加盟。")
		return
	}
	...
	_, err = dao.GetMemberByTeamIdUserId(team.Id, invite_user.Id)
	if err != nil {
...
func InviteMemberGet(w http.ResponseWriter, r *http.Request) {
	...
	report(w, s_u, "你好，慢条斯理的茶博士竟然说，茶团CEO或者创建人，才能发送该团邀请函呢。")
...
func MemberInvitationDetail(w http.ResponseWriter, r *http.Request) {
	...
	invi, err := dao.GetInvitationByUuid(invi_uuid)
	if err != nil {
		util.Debug(invi_uuid, "Cannot get invitation given uuid", err)
		report(w, s_u, "你好，茶博士正在努力的查找邀请函，请稍后再试。")
		return
	}

	// 读取目标茶团资料
	team, err := dao.GetTeam(invi.TeamId)
	if err != nil {
		util.Error("Cannot get team by id %d: %v", invi.TeamId, err)
		report(w, s_u, "你好，茶博士正在努力的查找邀请函资料，请稍后再试。")
		return
	}
	...
	if err != nil {
		util.Error("Cannot fetch invitationBean given invitation %d: %v", invi.Id, err)
		report(w, s_u, "你好，茶博士正在努力的查找邀请函资料，请稍后再试。")
		return
	}
	// 读取答复
	...
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		util.Error("Cannot get invitation reply %d: %v", invi.Id, err)
		report(w, s_u, "你好，茶博士正在努力的查找邀请函答复资料，请稍后再试。")
		return
	}
...
func MemberInvitationRead(w http.ResponseWriter, r *http.Request) {
	...
	invi, err := dao.GetInvitationByUuid(invi_uuid)
	if err != nil {
		util.Error("Cannot get invitation given uuid %s: %v", invi_uuid, err)
		report(w, s_u, "你好，茶博士正在努力的查找邀请函，请稍后再试。")
		return
	}
	...
	//if invi.InviteEmail != s_u.Email && !canManageTeam(&team, s_u, w) {
	if invi.InviteEmail != s_u.Email {
		report(w, s_u, "你好，该邀请函不属于您，无法查看。")
		return
	}
	...
	if invi.Status == dao.InvitationStatusPending {
		invi.Status = dao.InvitationStatusViewed
		err = invi.UpdateStatus()
		if err != nil {
			util.Error("Cannot update invitation %d: %v", invi.Id, err)
			report(w, s_u, "你好，茶博士正在努力的更新邀请函状态，请稍后再试。")
			return
		}
		// 减去茶友1小黑板新通知数
		if err = dao.SubtractUserNotificationCount(s_u.Id); err != nil {
			util.Error("Cannot subtract user notification count: %v", err)
			return
		}
...
func TeamNewApplicationsCheck(w http.ResponseWriter, r *http.Request) {
	...
	if uuid == "" {
		report(w, s_u, "你好，茶博士竟然说，陛下你没有给我茶团的uuid，请确认。")
		return
	}

	if uuid == dao.TeamUUIDFreelancer {
		report(w, s_u, "你好，茶博士竟然说，陛下你不能查看特殊茶团的加盟申请，请确认。")
		return
	}
	...
	if err != nil {
		util.Error("Cannot get team by given uuid %s: %v", uuid, err)
		report(w, s_u, "你好，茶博士失魂鱼，未能获取申请茶团，请稍后再试。")
		return
	}
	//检查用户是否茶团核心成员，非核心成员不能查看新的加盟申请书

	if !canManageTeam(&t_team, s_u, w) {
		report(w, s_u, "你好，茶博士竟然说，不是这个茶团核心成员不能查看，请确认。")
		return
	}

	//查询当前茶团未处理的加盟申请书，包含已查看但未处理的
	applies, err := dao.GetMemberApplicationByTeamIdAndStatus(t_team.Id)
	if err != nil {
		util.Error("Cannot get applys by team id %d: %v", t_team.Id, err)
		report(w, s_u, "你好，茶博士失魂鱼，未能获取申请茶团，请稍后再试。")
		return
	}
	apply_bean_slice, err := fetchMemberApplicationBeanSlice(applies)
	if err != nil {
		util.Error("Cannot get apply bean slice: %v", err)
		report(w, s_u, "你好，茶博士失魂鱼，未能获取申请茶团，请稍后再试。")
		return
	}
...
func TeamMemberResigned(w http.ResponseWriter, r *http.Request) {
	...
	resignations, err := dao.GetResignationsByTeamId(team.Id)
	if err != nil {
		util.Error("Cannot get resignations by team id %d: %v", team.Id, err)
		report(w, s_u, "你好，茶博士正在努力的查找退出声明，请稍后再试。")
		return
	}
	...
	var pageResignations []dao.TeamMemberResignation
	if totalCount > 0 {
	...
	type PageData struct {
		SessUser         dao.User
		Team             dao.Team
		ResignationSlice []dao.TeamMemberResignation
		CurrentPage      int
		TotalPages       int
		HasPrev          bool
		HasNext          bool
...
func TeamMemberResignationDetailGet(w http.ResponseWriter, r *http.Request) {
	...
	if resignation_uuid == "" {
		report(w, s_u, "你好，茶博士竟然说，陛下你没有给我退出声明的uuid，请确认。")
		return
	}
	resignation := dao.TeamMemberResignation{Uuid: resignation_uuid}
	if err = resignation.GetByUuid(); err != nil {
		util.Error("Cannot get resignation by uuid %s: %v", resignation_uuid, err)
		report(w, s_u, "你好，茶博士正在努力的查找退出声明，请稍后再试。")
		return
	}

	team, err := dao.GetTeam(resignation.TeamId)
	if err != nil {
		util.Error("Cannot get team by id %d: %v", resignation.TeamId, err)
		report(w, s_u, "你好，茶博士正在努力的查找退出声明资料，请稍后再试。")
		return
	}
	...
	member, err := dao.GetUser(resignation.MemberUserId)
	if err != nil {
		util.Error("Cannot get user by id %d: %v", resignation.MemberUserId, err)
		report(w, s_u, "你好，茶博士正在努力的查找退出声明资料，请稍后再试。")
		return
	}
	...
	memberCount, err := team.NumActiveMembers()
	if err != nil {
		util.Error("Cannot get team member count: %v", err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
	...
	if err != nil {
		util.Error("Cannot get CEO: %v", err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
	...
	if err != nil {
		util.Error("Cannot get core members: %v", err)
		report(w, s_u, "你好，茶博士正在忙碌中，稍后再试。")
		return
	}
	...
	for _, cm := range coreMembers {
		if cm.UserId == s_u.Id && cm.Role != dao.RoleCEO {
			isCoreMember = true
			break
		}
	}
	...
	type PageData struct {
		SessUser           dao.User
		Team               dao.Team
		Resignation        dao.TeamMemberResignation
		Member             dao.User
...
func InvitationsReceived(w http.ResponseWriter, r *http.Request) {
	...
	// 获取邀请函Bean列表
	invitationBeans := make([]dao.InvitationBean, 0)
	...
	type PageData struct {
		SessUser        dao.User
		InvitationBeans []dao.InvitationBean
		IsEmpty         bool
	}
...
func TeamMemberResignationProcess(w http.ResponseWriter, r *http.Request) {
	...
	if needDoubleApproval {
		...
		case "core_agree":
			// 核心成员同意
			if resignation.Status >= dao.ResignationStatusCoreMemberAgree {
				report(w, s_u, "你好，该声明已经处理过了。")
				return
			}
			// 检查是否为核心成员（非CEO）
			isCoreMember := false
			...
			if !isCoreMember {
				report(w, s_u, "你好，只有核心成员（非CEO）才能同意退出声明。")
				return
			}
			...
			report(w, s_u, "你好，已同意该成员退出，等待CEO批准。")

		case "ceo_approve":
			// CEO批准
			if ceoMember.UserId != s_u.Id {
				report(w, s_u, "你好，只有CEO才能批准退出声明。")
				return
			}
			if resignation.Status != dao.ResignationStatusCoreMemberAgree {
				report(w, s_u, "你好，需要核心成员先同意后，CEO才能批准。")
				return
			}
		...
		case "retain":
			...
			if resignation.Status >= dao.ResignationStatusApproved {
				report(w, s_u, "你好，该声明已经批准，无法挡留。")
				return
			}
			...
			report(w, s_u, "你好，已标记为挡留中，请与成员沟通。")

		default:
			report(w, s_u, "你好，无效的操作。")
		}
	} else {
		// 不需要双重审批，管理员直接处理
		if !canManageTeam(&team, s_u, w) {
