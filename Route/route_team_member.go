...
// MemberRole() GET /v1/team_member/role?id=
// 取出一张空白茶团成员角色任命书
func MemberRole(w http.ResponseWriter, r *http.Request) {
	// 读取会话
	s, err := Session(r)
	if err != nil {
	...
	if err != nil {
		util.Info(err, "Cannot get team by uuid")
		http.Redirect(w, r, "/v1/login", http.StatusFound)
		return
	}
...
func NewMemberApplication(w http.ResponseWriter, r *http.Request) {
	...
	//检查这个茶团是否已经存在该茶友
	_, err = data.GetTeamMemberByTeamIdAndUserId(team.Id, app_user.Id)
	if err != nil {
		switch err {
		case sql.ErrNoRows:
			break
		default:
			util.Danger(err, app_user.Email, " when checking team_member")
			Report(w, r, "你好，茶博士的眼镜被闪电破坏了，请稍后再试。")
			return
		}
	} else {
...
func MemberInvitationReply(w http.ResponseWriter, r *http.Request) {
	...
	if reply_class_int == 1 {
		...
		// 检查这个茶团是否已经存在该茶友了
		_, err = data.GetTeamMemberByTeamIdAndUserId(team.Id, reply_user.Id)
		if err != nil {
			switch err {
			case sql.ErrNoRows:
				break
...
func MemberInvitationRead(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Danger(err, invi.Id, " Cannot fetch invitationBean given invitation")
