...
func SignupAccount(w http.ResponseWriter, r *http.Request) {
	...
	if err := newU.Create(); err != nil {
		util.Danger(util.LogError(err), " Cannot create user")
	...
	if err = team_member.Create(); err != nil {
		util.Danger(util.LogError(err), " Cannot create default_free team_member")
	...
	if err = udt.Create(); err != nil {
		util.Danger(util.LogError(err), " Cannot create default team")
...
func Logout(w http.ResponseWriter, r *http.Request) {
	...
	if err != http.ErrNoCookie {
		...
		if ok {
			...
			err = sess.Delete()
			if err != nil {
				util.Danger(util.LogError(err), sess.Email, "Failed to delete session")
			}
			// else {
			//会话清除后的提示信息
			...
			util.Danger(util.LogError(err), sess.Email, " 登出时会话资料查询失败")
		}
