...
func OfficePilot(w http.ResponseWriter, r *http.Request) {
	...
	if user.Role == "pilot" || user.Role == "captain" {
		...
		if err != nil {
			util.Danger(err, " Cannot get pilots")
			http.Redirect(w, r, "/v1/", http.StatusFound)
		}
		util.GenerateHTML(w, &pilots, "layout", "navbar.private", "pilot.office")
	}
	//如果不是，则显示错误信息
	util.Report(w, r, "您好，欢迎光临茶博士服务室！")
}

// GET /pilot/new
// 返回添加飞行员页面
func NewPilot(w http.ResponseWriter, r *http.Request) {
	util.GenerateHTML(w, nil, "layout", "navbar.private", "pilot.new")
}
...
func AddPilot(w http.ResponseWriter, r *http.Request) {
	...
	if user.Role == "pilot" || user.Role == "captain" {
		...
		newPilot, err := data.UserByEmail(email)
		if err != nil {
			util.Danger(err, " Cannot find user by email")
			http.Redirect(w, r, "/v1/pilot/new", http.StatusFound)
			return
		}
		...
		if err := pilot.Create(); err != nil {
			util.Danger(err, " Cannot create pilot")
			http.Redirect(w, r, "/v1/pilot/new", http.StatusFound)
			return
		}
	}
