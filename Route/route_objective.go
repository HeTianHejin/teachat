...
func NewObjectivePost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.ScaldingTea(util.LogError(err), " cannot get count given objective title")
	...
	if err != nil {
		util.ScaldingTea(util.LogError(err), " cannot get count given objective team_id")
		Report(w, r, "你好，游丝软系飘春榭，落絮轻沾扑绣帘。请确认后再试。")
		return
	}
	// 最大团队可以创建 茶话会 数量
	...
	case 10:
		...
		if err = new_ob.Create(); err != nil {
			...
			// 记录错误，提示用户新开茶话会未成功
			util.ScaldingTea(util.LogError(err), " Cannot create objective")
	...
	case 20:
		...
		for _, t_id := range t_id_slice {
			...
			if err = obInviTeams.Create(); err != nil {
				...
				util.ScaldingTea(util.LogError(err), " Cannot create objectiveLicenseTeam")
			}
		}
	default:
		// 非法的茶话会属性
		util.ScaldingTea(util.LogError(err), " Unknown objective class")
	...
	if err = TwoAcceptMessagesSendExceptUserId(s_u.Id, mess); err != nil {
		util.ScaldingTea(util.LogError(err), "Cannot send 2 acceptMessage")
...
func ObjectiveSquare(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.ScaldingTea(util.LogError(err), " Cannot get objectives")
	...
	if err != nil {
		util.ScaldingTea(util.LogError(err), " Cannot read objective-bean slice")
...
func ObjectiveDetail(w http.ResponseWriter, r *http.Request) {
	...
	if err = RecordLastQueryPath(s_u.Id, r.URL.Path, r.URL.RawQuery); err != nil {
		util.ScaldingTea(util.LogError(err), s_u.Email, " Cannot record last query path")
	}
	// 用户足迹
