...
// 已超时来自团队的转账记录页面
func GetTeaTeamFromTeamExpiredTransfers(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	generateHTML(w, &pageData, "layout", "navbar.private", "tea.team.from_team_expired_transfers")
}

// 已超时来自用户的转账记录页面
func GetTeaTeamFromUserExpiredTransfers(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	generateHTML(w, &pageData, "layout", "navbar.private", "tea.team.from_user_expired_transfers")
}
