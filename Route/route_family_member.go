...
func FamilyMemberSignInNewGet(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		util.Info(util.LogError(err), "cannot get family by uuid")
		Report(w, r, "你好，柳丝榆荚自芳菲，不管桃飘与李飞。请稍后再试。")
		return
	}
...
func FamilyMemberSignInNewPost(w http.ResponseWriter, r *http.Request) {
	...
	if err = t_family.GetByUuid(); err != nil {
		//util.Warning(util.LogError(err), t_family.Uuid, "Cannot get family by uuid")
	...
	if isMember, err = t_family.IsMember(s_u.Id); err != nil || !isMember {
		util.Warning(util.LogError(err), s_u.Id, "Cannot check if user is member of family")
		Report(w, r, "你好，茶博士认为你不是这个家庭的成员，请确认后再试。")
		return
	}
	// 检查提及的茶友是否已经是提及的家庭的成员
	if isMember, err = t_family.IsMember(t_user.Id); isMember || err != nil {
		util.Warning(util.LogError(err), t_user.Id, "Cannot check if user is member of family")
	...
	if err != nil {
		util.Warning(util.LogError(err), t_family.Id, "Cannot get parent members of family")
	...
	if err = new_family_member_sign_in.Create(); err != nil {
		util.Warning(util.LogError(err), "Cannot create family member sign in")
...
func FamilyMemberSignInRead(w http.ResponseWriter, r *http.Request) {
	...
	if err := family_member_sign_in.GetByUuid(); err != nil {
		util.Danger(util.LogError(err), " Cannot get family_member_sign_in given uuid")
	...
	if err != nil {
		util.Danger(util.LogError(err), family_member_sign_in.Id, " Cannot get family_member_sign_in_bean")
		Report(w, r, "读取声明书失误，请稍后再试一次。")
		return
	}
	//更新声明书状态为已读
	...
	if err := family_member_sign_in.Update(); err != nil {
		util.Danger(util.LogError(err), " Cannot update family_member_sign_in")
...
func FamilyMemberSignInReply(w http.ResponseWriter, r *http.Request) {
	...
	if reply_bool {
		...
		if err = family_member.Create(); err != nil {
			util.Danger(util.LogError(err), " Cannot create family_member")
		...
		if family_member.Role == 1 || family_member.Role == 2 {
			...
			if err = family.Get(); err != nil {
				util.Danger(util.LogError(err), family.Id, " Cannot get family given id")
				Report(w, r, "你好，茶博士正在忙碌中，厚厚的眼镜不见了，稍后再试。")
				return
			}
			...
			if err = family.Update(); err != nil {
				util.Danger(util.LogError(err), " Cannot update family")
	...
	if err = family_member_sign_in_reply.Create(); err != nil {
		util.Danger(util.LogError(err), " Cannot create family_member_sign_in_reply")
